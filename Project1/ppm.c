...
struct ppm *ppm_read(const char *filename)
{
	...
	//cteni rozliseni a color bitratre
	unsigned int bitrate;
	int args;
	args = fscanf(fp, "%u %u %u", &PPM->xsize, &PPM->ysize, &bitrate);
	...
	//binarni data
	if (fread(PPM->data, sizeof(char), (3 * PPM->xsize * PPM->ysize), fp) !=
		(3 * PPM->xsize * PPM->ysize)) {
		if (ferror(fp) != 0) {
